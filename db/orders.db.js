...
const createOrderDb = async ({
  ...
  const { rows: order } = await pool.query(
    "INSERT INTO orders(customer_id, status, amount, total, ref, payment_method) VALUES($1, 'complete', $2, $3, $4, $5) returning *",
...
const getAllOrdersDb = async ({ userId, limit, offset }) => {
  ...
  const orders = await pool.query(
    `SELECT order_id, customer_id, status, date::date, amount, total 
      from orders WHERE orders.customer_id = $1 order by order_id desc limit $2 offset $3`,
    [userId, limit, offset]
  );
...
const getOrderDb = async ({ id, userId }) => {
  const { rows: order } = await pool.query(
    `SELECT product.*, order_item.quantity 
      from orders 
      join order_item
      on order_item.order_id = orders.order_id
      join product 
      on product.id = order_item.product_id 
      where orders.order_id = $1 AND orders.customer_id = $2`,
    [id, userId]
  );
  return order;
};
